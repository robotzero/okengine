...
vulkan_renderer_backend_initialize :: proc(backend: ^renderer_backend, application_name: string, plat_state: ^platform_state) -> bool {
  ...
  //     	vk.SubmitDebugUtilsMessageEXT(v_context.instance, {vk.DebugUtilsMessageSeverityFlagEXT.WARNING}, {vk.DebugUtilsMessageTypeFlagEXT.GENERAL}, &msg_callback_data);
	...
	v_context.swapchain.framebuffers = arr.darray_create(cast(u64)v_context.swapchain.image_count, vulkan_framebuffer)
	...
	// Create sync object.
	v_context.image_available_semaphores = arr.darray_create(cast(u64)v_context.swapchain.max_frames_in_flight, vk.Semaphore)
	v_context.queue_complete_semaphores = arr.darray_create(cast(u64)v_context.swapchain.max_frames_in_flight, vk.Semaphore)
	v_context.in_flight_fences = arr.darray_create(cast(u64)v_context.swapchain.max_frames_in_flight, vulkan_fence)
	...
	// by this list.
	v_context.images_in_flight = arr.darray_create(cast(u64)v_context.swapchain.image_count, ^vulkan_fence)
...
vulkan_renderer_backend_begin_frame :: proc(backend: ^renderer_backend, delta_time: f32) -> bool {
	...
	if v_context.recreating_swapchain {
		...
		if !vulkan_result_is_success(result) {
			log_error("vulkan renderer backend begin frame vkDeviceWaitIdle (1), failed: '%s'", vulkan_result_string(result, true))
			return false
		}
	...
	if v_context.framebuffer_size_generation != v_context.framebuffer_size_last_generation {
		...
		if !vulkan_result_is_success(result) {
			log_error("vulkan renderer backend begin frame vkDeviceWaitIdle (2), failed: '%s'", vulkan_result_string(result, true))
			return false
		}
	...
	vulkan_renderpass_begin(command_buffer, &v_context.main_renderpass, v_context.swapchain.framebuffers[v_context.image_index].handle)
...
vulkan_renderer_backend_end_frame :: proc(backend: ^renderer_backend, delta_time: f32) -> bool {
    ...
    v_context.images_in_flight[v_context.image_index] = &v_context.in_flight_fences[v_context.current_frame]
