...
vulkan_swapchain_acquire_next_image_index :: proc(
    ...
    swapchain: ^vulkan_swapchain,
    timeout_ns: u64,
    image_available_semaphore: vk.ImageSemaphore,
    fence: vk.Fence,
    out_image_index: ^u32) -> bool {

    vk.Result result = vk.AcquireNextImageKHR(
...
vulkan_swapchain_present :: proc(
    ...
    render_complete_semaphore: vk.Semapore, 
    ...
    if result == vk.Result.ERROR_OUT_OF_DATE_KHR || result == vk.Result.SUBOPTIMAL_KHR {
        ...
        vulkan_swapchain_recreate(context, context.framebuffer_width, context.framebuffer_height, swapchain)
    } else if (result != vk.Result_SUCCESS) {
...
create :: proc(v_context: ^vulkan_context, width: u32, height: u32, swapchain: ^vulkan_swapchain) {
    swapchain_extend : vk.Extent2D = {width, height}
...
destroy :: proc(vk_context: ^vulkan_context, swapchain: ^vulkan_swapchain) {
    vulkan_image_destroy(context, &swapchain.depth_attachment)

    // Only destroy the views, not the images, since those are owned by the swapchain and are thus
    // destroyed when it is.
    for view in swapchain.views {
        vk.DestroyImageView(context.device.logical_device, view, context.allocator)
    }

    vk.DestroySwapchainKHR(context.device.logical_device, swapchain.handle, context.allocator)
}
