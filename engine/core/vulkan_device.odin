...
vulkan_device_create :: proc(v_context: ^vulkan_context) -> bool {
	...
	transfer_shares_graphics_queue: bool =
		v_context.device.graphics_queue_index == v_context.device.transfer_queue_index
	index_count: i32 = 1
...
vulkan_device_query_swapchain_support :: proc(
	...
	out_support_info: ^vulkan_swapchain_support_info,
) {
	...
	if out_support_info.present_mode_count != 0 {
		...
		assert(
			vk.GetPhysicalDeviceSurfacePresentModesKHR(
				...
				surface,
				&out_support_info.present_mode_count,
				&out_support_info.present_modes[0],
			) ==
			vk.Result.SUCCESS,
		)
	}
...
select_physical_device :: proc(v_context: ^vulkan_context) -> bool {
	...
	assert(
		vk.EnumeratePhysicalDevices(v_context.instance, &physical_device_count, nil) ==
		vk.Result.SUCCESS,
	)
	...
	for dev in physical_devices {
		...
		result: bool = physical_device_meets_requirements(
			dev,
			v_context.surface,
			&properties,
			&features,
...
vulkan_device_detect_depth_format :: proc(device: ^vulkan_device) -> bool {
	...
	for candidate in candidates {
		...
		if vk.FormatFeatureFlag.DEPTH_STENCIL_ATTACHMENT in properties.linearTilingFeatures {
			...
			return true
		} else if vk.FormatFeatureFlag.DEPTH_STENCIL_ATTACHMENT in
