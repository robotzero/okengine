...
@(private = "file")
state_ptr2: ^texture_system2_state

create_default_textures2 :: proc(state: ^texture_system2_state) -> b8
destroy_default_textures2 :: proc(state: ^texture_system2_state)
load_texture2 :: proc(texture_name: string, t: ^texture) -> b8

texture_system2_initialize :: proc(
	memory_requirement: ^u64,
	state: rawptr,
	config: texture_system_config,
) -> b8 {
	if config.max_texture_count == 0 {
		log_fatal("texture_system2_initialize - config.max_texture_count must be > 0.")
		return false
	}
	...
	state_ptr2 = cast(^texture_system2_state)state
	state_ptr2.config = config

	// The array block is after the state. Already allocated, so just set the pointer.
	array_block := cast(^u8)state + struct_requirement
	state_ptr2.registered_textures = cast(^texture)array_block

	// Hashtable block is after array.
	hashtable_block := array_block + array_requirement

	// Create a hashtable for texture lookups.
	hashtable_create(
		size_of(texture_reference2),
		config.max_texture_count,
		hashtable_block,
		false,
		&state_ptr2.registered_texture_table,
	)

	// Fill the hashtable with invalid references to use as a default.
	invalid_ref: texture_reference2
	invalid_ref.auto_release = false
	invalid_ref.handle = INVALID_ID // Primary reason for needing default values.
	invalid_ref.reference_count = 0
	hashtable_fill(&state_ptr2.registered_texture_table, &invalid_ref)

	// Invalidate all textures in the array.
	for i: u32 = 0; i < state_ptr2.config.max_texture_count; i += 1 {
		state_ptr2.registered_textures[i].id = INVALID_ID
		state_ptr2.registered_textures[i].generation = INVALID_ID
	}

	// Create default textures for use in the system.
	create_default_textures2(state_ptr2)

	return true
}

texture_system2_shutdown :: proc(state: rawptr) {
	if state_ptr2 != nil {
		// Destroy all loaded textures.
		for i: u32 = 0; i < state_ptr2.config.max_texture_count; i += 1 {
			t := &state_ptr2.registered_textures[i]
			if t.generation != INVALID_ID {
				renderer_destroy_texture(t)
			}
		}

		destroy_default_textures2(state_ptr2)

		state_ptr2 = nil
	}
}

texture_system2_acquire :: proc(name: string, auto_release: b8) -> ^texture {
	...
	if strings.equal_fold(name, DEFAULT_TEXTURE_NAME) {
		log_warning(
			"texture_system2_acquire called for default texture. Use texture_system2_get_default_texture for texture 'default'.",
		)
		return &state_ptr2.default_texture
	}

	ref: texture_reference2
	if state_ptr2 != nil && hashtable_get(&state_ptr2.registered_texture_table, name, &ref) {
		...
		if ref.handle == INVALID_ID {
			...
			t: ^texture = nil
			for i: u32 = 0; i < state_ptr2.config.max_texture_count; i += 1 {
				if state_ptr2.registered_textures[i].id == INVALID_ID {
		...
		// Update the entry.
		hashtable_set(&state_ptr2.registered_texture_table, name, &ref)
		return &state_ptr2.registered_textures[ref.handle]
	}
...
texture_system2_release :: proc(name: string) {
	...
	if state_ptr2 != nil && hashtable_get(&state_ptr2.registered_texture_table, name, &ref) {
		...
		if ref.reference_count == 0 && ref.auto_release {
			t := &state_ptr2.registered_textures[ref.handle]
...
texture_system2_get_default_texture :: proc() -> ^texture {
	if state_ptr2 != nil {
		return &state_ptr2.default_texture
	}
