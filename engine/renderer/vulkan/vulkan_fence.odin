...
vulkan_fence_wait :: proc(
	...
	if fence.is_signaled == false {
		...
		case vk.Result.TIMEOUT:
			l.log_warning("vk_fence wait - Timed out")
		case vk.Result.ERROR_DEVICE_LOST:
			l.log_error("vk_fence_wait - VK_ERROR_DEVICE_LOST")
		case vk.Result.ERROR_OUT_OF_HOST_MEMORY:
			l.log_error("vk_fence_wait = VK_ERROR_OUT_OF_HOST_MEMORY")
		case vk.Result.ERROR_OUT_OF_DEVICE_MEMORY:
			l.log_error("vk_fence wait = VK_ERROR_OUT_OF_DEVICE_MEMORY")
		case:
			l.log_error("vk_fence_wait - An unknown error has occured.")
		}
	} else {
		res = true
	}
...
vulkan_fence_reset :: proc(v_context: ^vulkan_context, fence: ^vulkan_fence) {
	if fence.is_signaled {
		assert(
			vk.ResetFences(v_context.device.logical_device, 1, &fence.handle) == vk.Result.SUCCESS,
		)
		fence.is_signaled = false
	}
}
