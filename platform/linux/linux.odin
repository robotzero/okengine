...
platform_setup :: proc(plat_state: ^platform_state, application_name: cstring, x: i16, y: i16, width: u16, height: u16) -> bool {
    ...
    cookie: VoidCookie = create_window(state.connection, cast(u8) WindowClass.CopyFromParent, state.window, state.screen.root, x, y, width, height, 0, cast(u16) WindowClass.InputOutput, state.screen.rootVisual, event_values, &value_list[0])
